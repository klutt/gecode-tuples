...
The results confirm that tuple variables can be used to increase performance. The tuple variables with an exact domain perform better than the ones with an approximated domain. The latter ones perform almost identical to the integer reference. There is one case where the approximation is the best, and that is when the cost per transition is high. In most cases, the exact version is 80-\SI{100}{\percent} faster than the integer version used for reference.
