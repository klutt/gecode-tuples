...
public:
 Neq(Gecode::Space& home, MPG::IntPair::IntPairApproxView pv1, MPG::IntPair::IntPairApproxView pv2)
   : Gecode::Propagator::Propagator(home), p1(pv1), p2(pv2)
  {
  ...
  Neq(Gecode::Space& home, bool share, Neq& prop) {
    : Gecode::Propagator::Propagator(home, share, prop) {
...
//      std::cout << "Propagating Neq " << std::endl;
    if (p1.nq(home, p2.val()) == MPG::IntPair::ME_INTPAIRAPPROX_FAILED)
...
void neq(Gecode::Space& home, MPG::IntPairApproxVar p, MPG::IntPairApproxVar q) {
  //  std::cout << "Init Neq prop" << std::endl;
  MPG::IntPair::IntPairApproxView pv(p);
  MPG::IntPair::IntPairApproxView qv(q);
  if (Neq::post(home, pv, qv) != Gecode::ES_OK)
    home.fail();
}
